...
public class RestSiteSynchronizer : IDisposable
{
	...
	{
		public List<RestSiteOption> options = new List<RestSiteOption>();
	...
	private long _lastHoverMessageMsec;
	...
	private void TrySendHoverMessage()
	{
		...
		{
			int num = (int)(_lastHoverMessageMsec + 50 - Environment.TickCount64);
			if (num <= 0)
			{
	...
	private void SendHoverMessage()
	{
		if (_netService.IsConnected)
		{
			...
			_lastHoverMessageMsec = Environment.TickCount64;
