...
public class EventSynchronizer : IDisposable
{
	...
	public void BeginEvent(EventModel canonicalEvent, bool isPrefinished = false, Action<EventModel>? debugOnStart = null)
	{
		...
		foreach (Player player in _playerCollection.Players)
		{
			...
			eventModel.BeginEvent(player, isPrefinished);
	...
	private void ChooseOptionForEvent(Player player, int optionIndex)
	{
		...
		myEventOption eventOption = eventForPlayer.CurrentOptions[optionIndex];
		eventOption.Choose();
		SaveEventOptionToHistory(player, eventOption);
	}

	private void SaveEventOptionToHistory(Player player, myEventOption option)
	{
	...
	public EventModel GetLocalEvent()
	{
		...
		Player? localPlayer = _playerCollection.GetPlayer(_localPlayerId);
		if (localPlayer != null)
		{
		...
		throw new InvalidOperationException("Event is not in progress!");
	}
	...
	public EventModel GetEventForPlayer(Player player)
	{
		...
		int playerSlotIndex = _playerCollection.GetPlayerSlotIndex(player);
		if (playerSlotIndex < 0 || playerSlotIndex >= _events.Count)
		{
			_logger.Warn($"Player slot lookup failed for event {(_canonicalEvent?.Id ?? "(unknown)")}. Falling back to the first event instance.");
			return _events[0];
		}
