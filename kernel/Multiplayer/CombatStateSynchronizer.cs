...
public class CombatStateSynchronizer : IDisposable
{
	...
	private bool ShouldSyncOverNetwork => !IsDisabled && (_netService.Type == NetGameType.Host || _netService.Type == NetGameType.Client);

	public CombatStateSynchronizer(INetGameService netService, RunState runState)
	{
	...
	private void CheckSyncCompleted()
	{
		...
		if (_syncData.Count < _runState.Players.Count)
		{
			return;
		}
		if (_netService.Type == NetGameType.Host || _rngSet != null)
		{
			_syncCompletionSource.SetResult();
		}
	}
