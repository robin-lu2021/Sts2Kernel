...
public class CombatReplayWriter : IDisposable
{
	...
	public void WriteReplay(string filePath, bool stopRecording)
	{
		...
		string? directoryName = Path.GetDirectoryName(filePath.Replace('/', Path.DirectorySeparatorChar));
		if (!string.IsNullOrEmpty(directoryName))
		{
			Directory.CreateDirectory(directoryName);
		}
		using FileStream fileStream = File.Create(filePath.Replace('/', Path.DirectorySeparatorChar));
		fileStream.Write(_writer.Buffer, 0, _writer.BytePosition);
		if (stopRecording)
		{
			StopRecording();
		}
