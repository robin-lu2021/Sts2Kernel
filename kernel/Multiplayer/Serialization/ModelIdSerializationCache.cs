...
public static class ModelIdSerializationCache
{
	...
	public static void Init()
	{
		...
		List<Type> list = AbstractModelSubtypes.All.OrderBy(static type => type.Name, StringComparer.Ordinal).ToList();
		foreach (Type item7 in list)
		{
		...
		CategoryIdBitSize = CalculateBitSize(_netIdToCategoryNameMap.Count);
		EntryIdBitSize = CalculateBitSize(_netIdToEntryNameMap.Count);
		EpochIdBitSize = CalculateBitSize(_netIdToEpochNameMap.Count);
	...
	private static int CalculateBitSize(int count)
	{
		return (count > 1) ? (BitOperations.Log2((uint)(count - 1)) + 1) : 1;
	}
}
