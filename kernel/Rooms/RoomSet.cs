...
public class RoomSet
{
	...
	public void EnsureNextEventIsValid(RunState runState)
	{
		...
		{
			if (NextEvent.IsAllowed(runState) && !runState.VisitedEventIds.Any((ModelId visitedEventId) => visitedEventId == NextEvent.ModelId))
			{
				return;
			}
			eventsVisited++;
	...
	public SerializableRoomSet ToSave()
	{
		...
		{
			EventIds = events.Select((EventModel e) => e.ModelId).ToList(),
