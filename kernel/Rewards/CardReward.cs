...
public class CardReward : Reward
{
	...
	public override void Populate()
	{
		if (_cardsWereManuallySet)
		{
			...
			{
				Hook.AfterModifyingCardRewardOptions(base.Player.RunState, modifiers).GetAwaiter().GetResult();
			}
			return;
		}
		if (_cards.Count > 0)
	...
	protected override bool OnSelect()
	{
		...
		while (_cards.Count > 0)
		{
			...
			CardModel? result = CardSelectCmd.Selector?.GetSelectedCardReward(_cards, cardRewardOption)
				?? CardSelectCmd.FromChooseACardScreen(new BlockingPlayerChoiceContext(), _cards.Select((CardCreationResult entry) => entry.Card).ToList(), base.Player, canSkip: true);
			if (result == null)
			{
				removeReward = true;
				break;
			}
			CardPileAddResult cardPileAddResult = CardPileCmd.Add(result, PileType.Deck);
			...
			if (!Hook.ShouldAllowSelectingMoreCardRewards(base.Player.RunState, base.Player, this))
			{
				removeReward = true;
				break;
			}
