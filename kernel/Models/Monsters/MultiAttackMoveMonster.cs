...
public sealed class MultiAttackMoveMonster : MonsterModel
{
	...
	protected override MonsterMoveStateMachine GenerateMoveStateMachine()
	{
		...
		MoveState moveState = new MoveState("POKE", SyncMove(PokeMove), new MultiAttackIntent(1, 5));
		moveState.FollowUpState = moveState;
		list.Add(moveState);
		return new MonsterMoveStateMachine(list, moveState);
	}

	private void PokeMove(IReadOnlyList<Creature> targets)
	{
		DamageCmd.Attack(1m).WithHitCount(5).FromMonster(this)
			.Execute(null);
	}
}
