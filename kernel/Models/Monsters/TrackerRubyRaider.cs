...
public sealed class TrackerRubyRaider : MonsterModel
{
	...
	protected override MonsterMoveStateMachine GenerateMoveStateMachine()
	{
		...
		MoveState moveState = new MoveState("TRACK_MOVE", SyncMove(TrackMove), new DebuffIntent());
		MoveState moveState2 = (MoveState)(moveState.FollowUpState = new MoveState("HOUNDS_MOVE", SyncMove(HoundsMove), new MultiAttackIntent(HoundsDamage, HoundsRepeat)));
	...
	private void TrackMove(IReadOnlyList<Creature> targets)
	{
		PowerCmd.Apply<FrailPower>(targets, 2m, base.Creature, null);
	}

	private void HoundsMove(IReadOnlyList<Creature> targets)
	{
		DamageCmd.Attack(HoundsDamage).WithHitCount(HoundsRepeat).FromMonster(this)
			.Execute(null);
	}
}
