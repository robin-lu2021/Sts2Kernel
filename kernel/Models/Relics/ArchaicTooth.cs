...
public sealed class ArchaicTooth : RelicModel
{
	...
	{
		{
			KernelModelDb.Card<Bash>().Id,
			KernelModelDb.Card<Break>()
		},
		{
			KernelModelDb.Card<Neutralize>().Id,
			KernelModelDb.Card<Suppress>()
		},
		{
			KernelModelDb.Card<Unleash>().Id,
			KernelModelDb.Card<Protector>()
		},
		{
			KernelModelDb.Card<FallingStar>().Id,
			KernelModelDb.Card<MeteorShower>()
		},
		{
			KernelModelDb.Card<Dualcast>().Id,
			KernelModelDb.Card<Quadcast>()
		}
	};
	...
	private void UpdateHoverTips()
	{
		...
		if (StarterCard != null)
		{
			...
			_extraHoverTips.Add(KernelHoverTipFactory.FromCard(cardModel));
		...
		if (AncientCard != null)
		{
			...
			_extraHoverTips.Add(KernelHoverTipFactory.FromCard(cardModel2));
	...
	public override void AfterObtained()
	{
		...
		CardCmd.Transform(transcendenceStarterCard, GetTranscendenceTransformedCard(transcendenceStarterCard));
	}
}
