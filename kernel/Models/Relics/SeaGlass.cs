...
public class SeaGlass : RelicModel
{
	...
	public override void AfterObtained()
	{
		...
		foreach (CardModel item in CardSelectCmd.FromSimpleGridForRewards(prefs: new CardSelectorPrefs(RelicModel.L10NLookup(base.Id.Entry + ".selectionScreenPrompt"), 0, list.Count), context: new BlockingPlayerChoiceContext(), cards: list, player: base.Owner))
		{
			CardCmd.PreviewCardPileAdd(CardPileCmd.Add(item, PileType.Deck));
		}
	}
}