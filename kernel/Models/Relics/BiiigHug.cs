...
public sealed class BiiigHug : RelicModel
{
	...
	public override void AfterShuffle(PlayerChoiceContext choiceContext, Player shuffler)
	{
		if (shuffler == base.Owner)
		{
			CardModel soot = shuffler.Creature.CombatState.CreateCard<Soot>(base.Owner);
			CardPileCmd.AddGeneratedCardToCombat(soot, PileType.Draw, addedByPlayer: true, CardPilePosition.Random);
