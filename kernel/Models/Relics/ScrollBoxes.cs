...
public sealed class ScrollBoxes : RelicModel
{
	...
	{
		PlayerCmd.LoseGold(base.Owner.Gold, base.Owner);
		...
		foreach (CardModel item2 in CardSelectCmd.FromChooseABundleScreen(base.Owner, list2))
		{
			CardPileCmd.Add(item2, PileType.Deck);
		}
	}
	...
	public static List<IReadOnlyList<CardModel>> GenerateRandomBundles(Player player)
	{
		...
		for (int num = 0; num < 2; num++)
		{
			...
			{
				CardModel cardModel = KernelModelDb.Card<Claw>();
