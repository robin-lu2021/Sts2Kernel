...
public sealed class CallingBell : RelicModel
{
	...
	{
		CardPileCmd.AddCurseToDeck<CurseOfTheBell>(base.Owner);
		RewardsCmd.OfferCustom(base.Owner, GenerateRewards());
	}
	...
	private List<Reward> GenerateRewards()
	{
		...
		if (TestMode.IsOn)
		{
			...
			num2 = 0;
			span[num2] = new RelicReward(KernelModelDb.Relic<Anchor>().ToMutable(), base.Owner);
			num2++;
			span[num2] = new RelicReward(KernelModelDb.Relic<GremlinHorn>().ToMutable(), base.Owner);
			num2++;
			span[num2] = new RelicReward(KernelModelDb.Relic<MummifiedHand>().ToMutable(), base.Owner);
			return list;
		}
		num2 = 3;
