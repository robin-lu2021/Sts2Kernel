...
public sealed class PhilosophersStone : RelicModel
{
	...
	public override void AfterCreatureAddedToCombat(Creature creature)
	{
		...
		PowerCmd.Apply<StrengthPower>(creature, base.DynamicVars["StrengthPower"].BaseValue, null, null);
	}
	...
	public override void AfterRoomEntered(AbstractRoom room)
	{
		if (room is CombatRoom)
		{
			IEnumerable<Creature> targets = from c in base.Owner.Creature.CombatState.GetOpponentsOf(base.Owner.Creature)
				where c.IsAlive
				select c;
			PowerCmd.Apply<StrengthPower>(targets, base.DynamicVars["StrengthPower"].BaseValue, null, null);
		}
	}
}
