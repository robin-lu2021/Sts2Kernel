...
public sealed class WellLaidPlansPower : PowerModel
{
	...
	public override void BeforeFlushLate(PlayerChoiceContext choiceContext, Player player)
	{
		...
		List<CardModel> list = (CardSelectCmd.FromHand(prefs: new CardSelectorPrefs(base.SelectionScreenPrompt, 0, base.Amount), context: choiceContext, player: base.Owner.Player, filter: RetainFilter, source: this).ToList());
		if (list.Count == 0)
		{
			return;
		}
