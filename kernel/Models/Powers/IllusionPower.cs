...
public sealed class IllusionPower : PowerModel
{
	...
	public override void AfterApplied(Creature? applier, CardModel? cardSource)
	{
		...
		PowerCmd.Apply<MinionPower>(base.Owner, 1m, null, null);
	}
	...
	public override void AfterDeath(PlayerChoiceContext choiceContext, Creature creature, bool wasRemovalPrevented, float deathAnimLength)
	{
		if (!wasRemovalPrevented && creature == base.Owner)
		{
			...
			FollowUpStateId ??= base.Owner.Monster.MoveStateMachine.StateLog.Last().Id;
	...
	private void ReviveMove(IReadOnlyList<Creature> targets)
	{
		GetInternalData<Data>().isReviving = false;
		CreatureCmd.Heal(base.Owner, base.Owner.MaxHp - base.Owner.CurrentHp);
	}
}
