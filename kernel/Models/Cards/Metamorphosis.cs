...
public sealed class Metamorphosis : CardModel
{
	...
	{
		IEnumerable<CardModel> forCombat = KernelCardFactoryExtensions.GetForCombat(base.Owner, from c in base.Owner.Character.CardPool.GetUnlockedCards(base.Owner.UnlockState, base.Owner.RunState.CardMultiplayerConstraint)
		...
		foreach (CardModel item in forCombat)
		{
			...
			CardCmd.PreviewCardPileAdd(CardPileCmd.AddGeneratedCardToCombat(item, PileType.Draw, addedByPlayer: true, CardPilePosition.Random));
		}
	}
