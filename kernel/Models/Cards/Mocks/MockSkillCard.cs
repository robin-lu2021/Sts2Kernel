...
public sealed class MockSkillCard : MockCardModel
{
	...
	protected override void OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		{
			CreatureCmd.Damage(choiceContext, base.Owner.Creature, _mockSelfHpLoss, ValueProp.Unblockable | ValueProp.Unpowered | ValueProp.Move, this);
		}
		...
		{
			CardPileCmd.Draw(choiceContext, base.DynamicVars["Draw"].IntValue, base.Owner);
		}
		if (base.DynamicVars["Discard"].IntValue > 0)
		{
			CardCmd.Discard(choiceContext, CardSelectCmd.FromHandForDiscard(prefs: new CardSelectorPrefs(new LocString("cards", "MOCK_SKILL_CARD.discardSelectionPrompt"), base.DynamicVars["Discard"].IntValue), context: choiceContext, player: base.Owner, filter: null, source: this));
		}
		if (base.DynamicVars.Forge.BaseValue > 0m)
		{
			ForgeCmd.Forge(base.DynamicVars.Forge.BaseValue, base.Owner, this);
		}
		...
		foreach (PowerApplication application in _powerApplications)
		{
			...
			})
			{
				...
				PowerCmd.Apply(power, item, application.amount, base.Owner.Creature, this);
			}
		}
		foreach (CardCreation cardCreation in _cardCreations)
		{
			...
			if (!(canonicalCard is Shiv))
			{
				...
				{
					Soul.CreateInHand(base.Owner, cardCreation.amount, base.CombatState);
					continue;
				}
			...
			{
				Shiv.CreateInHand(base.Owner, cardCreation.amount, base.CombatState);
			}
		}
	}
