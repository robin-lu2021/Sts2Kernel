...
{
	private record struct Dish(string id, Action action, IEnumerable<IHoverTip> hoverTips, float weight)
	{
	...
	private void GrabSomethingOffTheBelt()
	{
	...
	private void ClamRoll()
	{
		CreatureCmd.Heal(base.Owner.Creature, base.DynamicVars["ClamRollHeal"].IntValue);
	}

	private void Caviar()
	{
		CreatureCmd.GainMaxHp(base.Owner.Creature, base.DynamicVars["CaviarMaxHp"].IntValue);
	}

	private void SuspiciousCondiment()
	{
	...
	private void GoldenFysh()
	{
		PlayerCmd.GainGold(base.DynamicVars["GoldenFyshGold"].BaseValue, base.Owner);
	}

	private void SpicySnappy()
	{
	...
	private void RollDish()
	{
		...
		if (NumOfGrabs % 5 == 0)
		{
			...
			_currentDish = new Dish("SEAPUNK_SALAD", SeapunkSalad, KernelHoverTipFactory.FromCardWithCardHoverTips<FeedingFrenzy>(), 0f);
			return;
		}
		int num = 4;
