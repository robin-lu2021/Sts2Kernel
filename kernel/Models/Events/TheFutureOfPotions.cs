...
public sealed class TheFutureOfPotions : EventModel
{
	...
	protected override IReadOnlyList<EventOption> GenerateInitialOptions()
	{
		...
		int num = Math.Min(3, list2.Count);
		for (int i = 0; i < num; i++)
		{
			...
			list.Add(new EventOption(this, () => Trade(potion), choiceTitle, choiceDescription, "THE_FUTURE_OF_POTIONS.pages.INITIAL.options.POTION", potion.HoverTips).ThatHasDynamicTitle());
		}
		return list;
	}

	private void Trade(PotionModel potion)
	{
		...
		RewardsCmd.OfferCustom(base.Owner, new List<Reward>(1) { reward });
		Done();
