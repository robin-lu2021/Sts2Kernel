...
public sealed class Symbiote : EventModel
{
	...
	{
		CardModel cardModel = RunSynchronously(CardSelectCmd.FromDeckForEnchantment(prefs: new CardSelectorPrefs(CardSelectorPrefs.EnchantSelectionPrompt, 1), player: base.Owner, enchantment: ModelDb.Enchantment<Corrupted>(), amount: 1).FirstOrDefault());
		if (cardModel != null)
		{
	...
	{
		List<CardModel> list = RunSynchronously(CardSelectCmd.FromDeckForTransformation(prefs: new CardSelectorPrefs(CardSelectorPrefs.TransformSelectionPrompt, base.DynamicVars.Cards.IntValue), player: base.Owner).ToList());
