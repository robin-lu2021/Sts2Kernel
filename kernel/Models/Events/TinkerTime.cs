...
public sealed class TinkerTime : EventModel
{
	...
	private void ChooseRiderEffect()
	{
	...
	private void RiderChosen(RiderEffect rider)
	{
		MadScience madScience = base.Owner.RunState.CreateCard<MadScience>(base.Owner);
		madScience.TinkerTimeType = ChosenCardType;
		madScience.TinkerTimeRider = rider;
		CardPileCmd.Add(madScience, PileType.Deck);
	...
	public static IHoverTip[] GetRiderHoverTips(RiderEffect rider)
	{
		return rider switch
		{
			...
			{
				KernelHoverTipFactory.FromPower<WeakPower>(),
				KernelHoverTipFactory.FromPower<VulnerablePower>()
			}, 
			RiderEffect.Violence => Array.Empty<IHoverTip>(), 
			RiderEffect.Choking => new IHoverTip[1] { KernelHoverTipFactory.FromPower<StranglePower>() }, 
			...
			{
				KernelHoverTipFactory.FromPower<StrengthPower>(),
				KernelHoverTipFactory.FromPower<DexterityPower>()
			}, 
