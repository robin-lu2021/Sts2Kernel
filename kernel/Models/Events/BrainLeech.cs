...
public sealed class BrainLeech : EventModel
{
	...
	private void Rip()
	{
		CreatureCmd.Damage(new ThrowingPlayerChoiceContext(), base.Owner.Creature, (DamageVar)base.DynamicVars["RipHpLoss"], null, null);
		for (int i = 0; i < base.DynamicVars["RewardCount"].IntValue; i++)
		{
			...
			RewardsCmd.OfferCustom(base.Owner, new List<Reward>(1) { item });
		}
	...
	private void ShareKnowledge()
	{
		...
		CardModel cardModel = (CardSelectCmd.FromSimpleGridForRewards(new BlockingPlayerChoiceContext(), cards, base.Owner, prefs).FirstOrDefault());
		if (cardModel != null)
		{
			CardPileCmd.Add(cardModel, PileType.Deck);
		}
