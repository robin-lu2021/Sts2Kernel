...
public sealed class SpiralingWhirlpool : EventModel
{
	...
	private void ObserveTheSpiral()
	{
		CardModel? cardModel = CardSelectCmd.FromDeckForEnchantment(base.Owner, ModelDb.Enchantment<Spiral>(), 1, (CardModel? c) => ModelDb.Enchantment<Spiral>().CanEnchant(c), new CardSelectorPrefs(CardSelectorPrefs.EnchantSelectionPrompt, 1)).FirstOrDefault();
		if (cardModel != null)
		{
