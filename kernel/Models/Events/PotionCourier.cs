...
public sealed class PotionCourier : EventModel
{
	...
	protected override IReadOnlyList<EventOption> GenerateInitialOptions()
	{
		...
		{
			new EventOption(this, GrabPotions, "POTION_COURIER.pages.INITIAL.options.GRAB_POTIONS", KernelHoverTipFactory.FromPotion<FoulPotion>()),
	...
	private void GrabPotions()
	{
		List<Reward> list = new List<Reward>();
		for (int i = 0; i < base.DynamicVars["FoulPotions"].IntValue; i++)
		{
			list.Add(new PotionReward(KernelModelDb.Potion<FoulPotion>().ToMutable(), base.Owner));
		}
