...
public sealed class SlipperyBridge : EventModel
{
	...
	protected override IReadOnlyList<EventOption> GenerateInitialOptions()
	{
		...
		{
			new EventOption(this, Overcome, "SLIPPERY_BRIDGE.pages.INITIAL.options.OVERCOME", KernelHoverTipFactory.FromCard(RandomCardToLose)),
	...
	{
		List<CardModel> list = ((RandomCardToLose != null) ? 
		base.Owner.Deck.Cards.Where((CardModel c) => c.GetType() != RandomCardToLose.GetType()).ToList() : 
		base.Owner.Deck.Cards.Where((CardModel c) => c.Rarity != CardRarity.Basic).ToList());
	...
	{
		CardPileCmd.RemoveFromDeck(RandomCardToLose, showPreview: false);
		SetEventFinished(L10NLookup("SLIPPERY_BRIDGE.pages.OVERCOME.description"));
	}

	private void HoldOn()
	{
		CreatureCmd.Damage(new ThrowingPlayerChoiceContext(), base.Owner.Creature, CurrentHpLoss, ValueProp.Unblockable | ValueProp.Unpowered, null, null);
		NumberOfHoldOns++;
