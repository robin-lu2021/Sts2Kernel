...
public sealed class SunkenTreasury : EventModel
{
	...
	private void FirstChest()
	{
		PlayerCmd.GainGold(base.DynamicVars["SmallChestGold"].BaseValue, base.Owner);
		SetEventFinished(L10NLookup("SUNKEN_TREASURY.pages.FIRST_CHEST.description"));
	}

	private void SecondChest()
	{
		PlayerCmd.GainGold(base.DynamicVars["LargeChestGold"].BaseValue, base.Owner);
		CardPileCmd.AddCurseToDeck<Greed>(base.Owner);
