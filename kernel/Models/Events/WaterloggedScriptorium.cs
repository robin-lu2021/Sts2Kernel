...
public sealed class WaterloggedScriptorium : EventModel
{
	...
	private void PricklySponge()
	{
		PlayerCmd.LoseGold(base.DynamicVars["PricklySpongeGold"].BaseValue, base.Owner, GoldLossType.Spent);
		...
		foreach (CardModel item in RunSynchronously(CardSelectCmd.FromDeckForEnchantment(base.Owner, enchantment, 1, prefs)))
		{
	...
	private void TentacleQuill()
	{
		PlayerCmd.LoseGold(base.DynamicVars.Gold.BaseValue, base.Owner, GoldLossType.Spent);
		CardModel cardModel = RunSynchronously(CardSelectCmd.FromDeckForEnchantment(prefs: new CardSelectorPrefs(CardSelectorPrefs.EnchantSelectionPrompt, 1), player: base.Owner, enchantment: ModelDb.Enchantment<Steady>(), amount: 1).FirstOrDefault());
		if (cardModel != null)
		{
