...
public sealed class DollRoom : EventModel
{
	...
	private static readonly DollChoice[] _dolls = new DollChoice[3]
	{
		new DollChoice
		{
			relic = KernelModelDb.Relic<DaughterOfTheWind>(),
			descriptionKey = "DOLL_ROOM.pages.DAUGHTER_OF_WIND.description"
		},
		new DollChoice
		{
			relic = KernelModelDb.Relic<MrStruggles>(),
			descriptionKey = "DOLL_ROOM.pages.MR_STRUGGLES.description"
		},
		new DollChoice
		{
			relic = KernelModelDb.Relic<BingBong>(),
	...
	{
		ChooseDollAndShowDescription(base.Rng.NextItem(_dolls));
	}

	private void TakeSomeTime()
	{
		CreatureCmd.Damage(new ThrowingPlayerChoiceContext(), base.Owner.Creature, (DamageVar)base.DynamicVars["TakeTimeHpLoss"], null, null);
	...
	{
		CreatureCmd.Damage(new ThrowingPlayerChoiceContext(), base.Owner.Creature, (DamageVar)base.DynamicVars["ExamineHpLoss"], null, null);
	...
	private EventOption OptionFromChoice(DollChoice choice)
	{
		...
		return (EventOption)new EventOption(this, Func, title, locString, choice.relic.Title.GetRawText(), KernelHoverTipFactory.FromRelic(choice.relic)).WithOverridenHistoryName(choice.relic.Title);
		void Func()
		{
			ChooseDollAndShowDescription(choice);
		}
	}

	private void ChooseDollAndShowDescription(DollChoice choice)
	{
		RelicCmd.Obtain(choice.relic.ToMutable(), base.Owner);
