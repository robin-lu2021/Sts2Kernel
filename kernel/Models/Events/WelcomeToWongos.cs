...
public sealed class WelcomeToWongos : EventModel
{
	...
	private LocString CheckObtainWongoBadge(int pointsEarned)
	{
		...
		if (num2 >= 2000)
		{
			RelicCmd.Obtain<WongoCustomerAppreciationBadge>(base.Owner);
	...
	private void BuyBargainBin()
	{
		PlayerCmd.LoseGold(base.DynamicVars["BargainBinCost"].BaseValue, base.Owner, GoldLossType.Spent);
		...
		SetEventFinished(CheckObtainWongoBadge(32));
	}

	private void BuyMysteryBox()
	{
		PlayerCmd.LoseGold(base.DynamicVars["MysteryBoxCost"].BaseValue, base.Owner, GoldLossType.Spent);
		RelicCmd.Obtain<WongosMysteryTicket>(base.Owner);
		SetEventFinished(CheckObtainWongoBadge(8));
	}

	private void BuyFeaturedItem()
	{
		PlayerCmd.LoseGold(base.DynamicVars["FeaturedItemCost"].BaseValue, base.Owner, GoldLossType.Spent);
		if (FeaturedItem != null)
		{
			RelicCmd.Obtain(FeaturedItem.ToMutable(), base.Owner);
		}
		SetEventFinished(CheckObtainWongoBadge(16));
	}

	private void Leave()
	{
		Player owner = base.Owner;
		CardModel cardModel = base.Rng.NextItem(owner.Deck.Cards.OfType<CardModel>().Where(c => c.IsUpgraded));
		if (cardModel != null)
		{
