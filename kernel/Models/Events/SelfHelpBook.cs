...
public sealed class SelfHelpBook : EventModel
{
	...
	private void ReadTheBack()
	{
		SelectAndEnchant<Sharp>(2, CardType.Attack, L10NLookup("SELF_HELP_BOOK.pages.READ_THE_BACK.description"));
	}

	private void ReadPassage()
	{
		SelectAndEnchant<Nimble>(2, CardType.Skill, L10NLookup("SELF_HELP_BOOK.pages.READ_PASSAGE.description"));
	}

	private void ReadEntireBook()
	{
		SelectAndEnchant<Swift>(2, CardType.Power, L10NLookup("SELF_HELP_BOOK.pages.READ_ENTIRE_BOOK.description"));
	}
	...
	private bool PlayerHasCardsAvailable<T>(Player player, CardType typeRestriction) where T : EnchantmentModel
	{
		...
		return PileType.Deck.GetPile(player).Cards.Any((CardModel c) => DeckFilter(c, enchantment, typeRestriction) != null);
	}

	private void SelectAndEnchant<T>(int amount, CardType typeRestriction, LocString finalDescription) where T : EnchantmentModel
	{
		CardSelectorPrefs prefs = new CardSelectorPrefs(CardSelectorPrefs.EnchantSelectionPrompt, 1);
		EnchantmentModel enchantment = ModelDb.Enchantment<T>();
		CardModel cardModel = CardSelectCmd.FromDeckForEnchantment(base.Owner, enchantment, amount, (CardModel? c) => c.Type == typeRestriction, prefs).FirstOrDefault();
		if (cardModel != null)
		{
			ApplyEnchantment<T>(cardModel, amount);
		}
	...
	private void ApplyEnchantment<T>(CardModel card, int amount) where T : EnchantmentModel
	{
		CardCmd.Enchant<T>(card, amount);
	}

	private void SkipBook()
	{
