...
public sealed class BattlewornDummy : EventModel
{
	...
	{
		new DynamicVar("Setting1Hp", KernelModelDb.Monster<BattleFriendV1>().MinInitialHp),
		new DynamicVar("Setting2Hp", KernelModelDb.Monster<BattleFriendV2>().MinInitialHp),
		new DynamicVar("Setting3Hp", KernelModelDb.Monster<BattleFriendV3>().MinInitialHp)
	});
	...
	protected override IReadOnlyList<EventOption> GenerateInitialOptions()
	{
		...
		base.DynamicVars["Setting1Hp"].BaseValue = Creature.ScaleHpForMultiplayer(KernelModelDb.Monster<BattleFriendV1>().MinInitialHp, ModelDb.Encounter<BattlewornDummyEventEncounter>(), playerCount, actIndex);
		base.DynamicVars["Setting2Hp"].BaseValue = Creature.ScaleHpForMultiplayer(KernelModelDb.Monster<BattleFriendV2>().MinInitialHp, ModelDb.Encounter<BattlewornDummyEventEncounter>(), playerCount, actIndex);
		base.DynamicVars["Setting3Hp"].BaseValue = Creature.ScaleHpForMultiplayer(KernelModelDb.Monster<BattleFriendV3>().MinInitialHp, ModelDb.Encounter<BattlewornDummyEventEncounter>(), playerCount, actIndex);
	...
	public override void Resume(AbstractRoom room)
	{
		...
		{
			RelicModel relic = RelicModel.FromCore(RelicFactory.PullNextRelicFromFront(base.Owner).ToMutable());
