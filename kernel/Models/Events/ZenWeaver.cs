...
public sealed class ZenWeaver : EventModel
{
	...
	protected override IReadOnlyList<EventOption> GenerateInitialOptions()
	{
		...
		EventOption eventOption = new EventOption(this, BreathingTechniques, "ZEN_WEAVER.pages.INITIAL.options.BREATHING_TECHNIQUES");
	...
	private void BreathingTechniques()
	{
		PlayerCmd.LoseGold(base.DynamicVars["BreathingTechniquesCost"].IntValue, base.Owner, GoldLossType.Spent);
		CardModel[] array = new CardModel[2];
		for (int i = 0; i < array.Length; i++)
		{
			array[i] = base.Owner.RunState.CreateCard<Enlightenment>(base.Owner);
		}
		CardPileCmd.Add(array, PileType.Deck);
		SetEventFinished(L10NLookup("ZEN_WEAVER.pages.BREATHING_TECHNIQUES.description"));
	}

	private void EmotionalAwareness()
	{
		RemoveCardsAndProceed(base.DynamicVars["EmotionalAwarenessCost"].IntValue, 1);
		SetEventFinished(L10NLookup("ZEN_WEAVER.pages.EMOTIONAL_AWARENESS.description"));
	}

	private void ArachnidAcupuncture()
	{
		RemoveCardsAndProceed(base.DynamicVars["ArachnidAcupunctureCost"].IntValue, 2);
		SetEventFinished(L10NLookup("ZEN_WEAVER.pages.ARACHNID_ACUPUNCTURE.description"));
	}

	private void RemoveCardsAndProceed(int cost, int count)
	{
		CardPileCmd.RemoveFromDeck(CardSelectCmd.FromDeckForRemoval(base.Owner, new CardSelectorPrefs(CardSelectorPrefs.RemoveSelectionPrompt, count)).ToList());
		PlayerCmd.LoseGold(cost, base.Owner, GoldLossType.Spent);
	}
