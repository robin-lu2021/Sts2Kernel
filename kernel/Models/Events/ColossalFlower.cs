...
public sealed class ColossalFlower : EventModel
{
	...
	private void ReachDeeper()
	{
		DealReachDeeperDamage();
		NumberOfDigs++;
		if (NumberOfDigs < 2)
		{
		...
		else
		{
			SetEventState(L10NLookup("COLOSSAL_FLOWER.pages.REACH_DEEPER_2.description"), new global::_003C_003Ez__ReadOnlyArray<EventOption>(new EventOption[2]
			{
				...
				new EventOption(this, ObtainPollinousCore, "COLOSSAL_FLOWER.pages.REACH_DEEPER_2.options.POLLINOUS_CORE", KernelHoverTipFactory.FromRelic<PollinousCore>().ThatDoesDamage(_prizeDamage[NumberOfDigs]))
			}));
		}
	}

	private void ExtractCurrentPrize()
	{
		PlayerCmd.GainGold(_prizeCosts[NumberOfDigs], base.Owner);
		SetEventFinished(L10NLookup("COLOSSAL_FLOWER.pages.EXTRACT_CURRENT_PRIZE.description"));
	}

	private void ExtractInstead()
	{
		PlayerCmd.GainGold(_prizeCosts[NumberOfDigs], base.Owner);
		SetEventFinished(L10NLookup("COLOSSAL_FLOWER.pages.EXTRACT_INSTEAD.description"));
	}

	private void ObtainPollinousCore()
	{
		DealReachDeeperDamage();
		RelicCmd.Obtain<PollinousCore>(base.Owner);
		SetEventFinished(L10NLookup("COLOSSAL_FLOWER.pages.POLLINOUS_CORE.description"));
	}

	private void DealReachDeeperDamage()
	{
		CreatureCmd.Damage(new ThrowingPlayerChoiceContext(), base.Owner.Creature, _prizeDamage[NumberOfDigs], ValueProp.Unblockable | ValueProp.Unpowered, null, null);
	}
}
