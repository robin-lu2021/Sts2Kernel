...
public static class CardFactory
{
	...
	public static CardCreationResult CreateForMerchant(Player player, IEnumerable<CardModel> options, CardType type)
	{
		...
		var preFilterPool = Hook.ModifyMerchantCardPool(player.RunState, player, options).Where((CardModel c) => c.Rarity != CardRarity.Basic).ToList();
		var postFilterPool = FilterForPlayerCount(player.RunState, preFilterPool).ToArray();
		options = postFilterPool;
	...
	private static CardModel CreateForReward(Player player, IEnumerable<CardModel> blacklist, CardCreationOptions options)
	{
		...
		var preFilterPool = options.GetPossibleCards(player).Except(blacklist).ToList();
		IEnumerable<CardModel> options2 = FilterForPlayerCount(player.RunState, preFilterPool).ToArray();
