...
public abstract class MerchantEntry
{
	...
	public bool OnTryPurchaseWrapper(MerchantInventory? inventory, bool ignoreCost = false)
	{
		if (!IsStocked)
		{
		...
		var (success, goldSpent) = OnTryPurchase(inventory, ignoreCost);
		if (success)
		{
	...
	protected abstract (bool, int) OnTryPurchase(MerchantInventory? inventory, bool ignoreCost);
