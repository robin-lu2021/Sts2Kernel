...
public sealed class MerchantPotionEntry : MerchantEntry
{
	...
	public override void CalcCost()
	{
		...
		if (TestMode.IsOff)
		{
			_cost = (int)Math.Round((double)_cost * _player.PlayerRng.Shops.NextFloat(0.95f, 1.05f));
		}
	}
	...
	protected override (bool, int) OnTryPurchase(MerchantInventory? inventory, bool ignoreCost)
	{
		...
		PotionProcureResult potionProcureResult = PotionCmd.TryToProcure(Model, _player);
