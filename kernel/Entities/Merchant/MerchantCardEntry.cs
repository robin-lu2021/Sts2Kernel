...
public sealed class MerchantCardEntry : MerchantEntry
{
	...
	private static int GetCost(CardModel card)
	{
		...
		{
			num = (int)Math.Round((double)num * 1.15);
		}
		return num;
	}
	...
	public override void CalcCost()
	{
		...
		_cost = (int)Math.Round((double)GetCost(CreationResult.Card) * _player.PlayerRng.Shops.NextFloat(0.95f, 1.05f));
		if (IsOnSale)
		{
			_cost /= 2;
		}
	...
	{
		if (!CardPileCmd.Add(CreationResult.Card, PileType.Deck).success)
		{
		...
		if (!ignoreCost)
		{
			PlayerCmd.LoseGold(base.Cost, _player, GoldLossType.Spent);
		}
