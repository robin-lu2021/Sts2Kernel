...
public class Player
{
	...
	private void LoadInventory(SerializablePlayer save)
	{
		...
		PopulateRelics(save.Relics.Select(static relicSave => RelicModel.FromSerializable(relicSave)));
	}
	...
	public SerializablePlayer ToSerializable()
	{
		return new SerializablePlayer
		{
			...
			Deck = Deck.Cards.Select(static (CardModel c) => (SerializableCard)c.ToSerializable()).ToList(),
			Relics = Relics.Select(static (RelicModel r) => (SerializableRelic)(MegaCrit.Sts2.Core.Saves.Runs.mySerializableRelic)r.ToSerializable()).ToList(),
			Potions = PotionSlots.Select(static (PotionModel? p, int i) => p == null ? null : (SerializablePotion)p.ToSerializable(i)).OfType<SerializablePotion>().ToList(),
	...
	public void SyncWithSerializedPlayer(SerializablePlayer player)
	{
		...
		PopulateRelics(player.Relics.Select(static relicSave => RelicModel.FromSerializable(relicSave)), silent: true);
	...
	public Task ReviveBeforeCombatEnd()
	{
		if (Creature.IsDead)
		{
			CreatureCmd.Heal(Creature, 1m);
		}
